...
def sarsa(env, n_episodes, gamma=1.0, alpha=0.5, epsilon=0.1):
    ...
    for i in range(n_episodes):
        ...
        action = epsilon_greedy(Q,ST,env.action_space.n,epsilon)
        while True:
            state, reward, done, _, _ = env.step(action)
            action1 = epsilon_greedy(Q,state,env.action_space.n,epsilon)
            Q[ST][action] = Q[ST][action] + alpha*(reward + gamma *(Q[state][action1]) - Q[ST][action])
            if done:
                break
            action = action1
            ST = state
...
def q_learning(env, n_episodes, gamma=1.0, alpha=0.5, epsilon=0.1):
    ...
    for i in range(n_episodes):
        ...
        while True:
            ...
            Q[ST][action] = Q[ST][action] + alpha*(reward + gamma*np.max(Q[new_state]) - Q[ST][action])
            ST = new_state
            if done:
                break
